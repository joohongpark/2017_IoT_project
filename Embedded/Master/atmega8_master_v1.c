...
volatile unsigned char flag = 1, status_now, rx_exist, rx_prev_exist;
...
void xprintf(unsigned char *str_data) {
	while(*str_data != 0x00){ //문자열의 끝부분이 아니라면
		tx_char(*str_data); //시리얼포트로 한개의 문자를 송신한다.
		str_data++;
		}
}
void xprintf_hex(unsigned char n, unsigned char *str_data) {
	int i;
	for(i = 0; i<n; i++){
		tx_hex(str_data[i]);
		tx_char(' ');
...
void tx_hex(unsigned char data) {
	tx_char( (data / 16) < 0x0a ? ( (data / 16) + 0x30 ) : ( (data / 16) + 0x37) );
	tx_char( (data % 16) < 0x0a ? ( (data % 16) + 0x30 ) : ( (data % 16) + 0x37) );
}

void tx_dec(unsigned char data) {
...
volatile unsigned char rx[32] = {0, };
volatile unsigned int recv_PERIOD = 0x0000; // 2byte
volatile unsigned long recv_RAENERGY = 0x00000000; // 3byte
volatile unsigned char recv_buf = 0x00; // 1byte
volatile unsigned int i = 1, k;
volatile unsigned char eeprom_pointer = 0x00;
volatile unsigned char transmit_buf = 0x00;
volatile unsigned char uart_rx_buf = 0x00;
volatile unsigned char is_exist = false;
volatile unsigned char tx_dump[2] = {0x00, 0x00};

volatile unsigned char rx_mode = false;
volatile unsigned char buf_pointer = 0x00;
volatile unsigned char ctrl_addr = 0x00;
volatile unsigned char ctrl_value = 0x00;
...
int main(void) {
	...
	RF_write_addr(RX_ADDR_P1, 0xD728D728D7);


	i = 0x01; // start addr
	while(1) {
		addr = ((uint64_t)i << 32) + ((uint64_t)i << 24) + ((uint64_t)i << 16) + ((uint64_t)i << 8) + i;
		...
		tx_dump[0] = (i << 2) | (transmit_buf & 0x01);
		tx_dump[1] = i + 5;
		RF_tran_payload((char*) &tx_dump, 2); // 데이터 전송
		CE1;
		_delay_us(12);
		CE0;		
		flag = true;
		...
		if(rx_exist){
			/*
			dump = ((unsigned long)rx[0] << 24) | ((unsigned long)rx[1] << 16) | ((unsigned long)rx[2] << 8) | rx[3];
			tx_hex(i);
			xprintf(" : ");
			print_long_dec(dump);
			*/
			...
			if(transmit_buf == true)
				...
				xprintf("0");
			xprintf(",\"RAENERGY\":");
			print_long_dec_without_zero(recv_RAENERGY);
			xprintf(",\"PERIOD\":");
			print_long_dec_without_zero(recv_PERIOD);
			xprintf(",\"raw_data\":");
			for (k = 0; k < 32; k++)
			{
				tx_hex(rx[k]);
