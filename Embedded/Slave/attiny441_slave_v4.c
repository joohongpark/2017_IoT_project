...
void RF_init(void)
{
	...
	RF_rw_1b(RF_RX_PW_P1, 0x04, true); // 파이프라인 1번 수신
...
volatile unsigned char recv_chr[4];
...
volatile uint64_t addrf = ((uint64_t)DEV_ADDR << 32) | ((uint64_t) (~DEV_ADDR & 0xff) << 24) | ((uint64_t)DEV_ADDR << 16) | ((uint64_t) (~DEV_ADDR & 0xff) << 8) | DEV_ADDR;
...
int main(void) {
	...
	RF_write_addr(RF_TX_ADDR, 0x5050050555);
	RF_write_addr(RF_RX_ADDR_P0, 0);
	RF_write_addr(RF_RX_ADDR_P1, addrf);
...
ISR(INT0_vect) {
	sei();
	if(flag == true) {
		RF_recv_payload((char*) &recv_chr, 4);
		if( ((recv_chr[0] >> 4) == DEV_ADDR) && ((recv_chr[0] >> 4) ^ (recv_chr[0] & 0x0f) == 0x0f) ) {
		//if( true ) {
			if( (recv_chr[1] & 0x01) == true) {
				OUTPUT_high;
			} else {
				OUTPUT_low;
			}
			...
			recv_pay[2] = recv_chr[2];
			recv_pay[3] = recv_chr[3];
			for (jj = 0; jj < 14; jj++)
			{
				recv_pay[jj+4] = dump[0][jj];
			}
			/*
			if(is_read == true) {
				test[0] = DEV_ADDR;
			...
			*/
			...
			_delay_us(160);
			RF_write_addr(RF_RX_ADDR_P0, 0x5050050555); // 마스터 어드레스를 수신 어드레스로 지정
			RF_rw_1b(RF_STATUS, 0b01110000, true); // rx_dr, tx_ds, max_rt 초기화	
